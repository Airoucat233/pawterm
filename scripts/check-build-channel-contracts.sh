...
grep -q 'serverTag = serverUpdateChannel == .prerelease ? "prerelease" : "latest"' mac/Sources/PawTerm/ServerManager.swift || fail "Mac prerelease channel must check npm server prerelease dist-tag"
grep -q 'serverPackage = serverUpdateChannel == .prerelease ? "pawterm-server@prerelease" : "pawterm-server@latest"' mac/Sources/PawTerm/ServerManager.swift || fail "Mac prerelease channel must install npm server prerelease dist-tag"
grep -q 'Mac App prerelease' mac/Sources/PawTerm/MenuBarContent.swift || fail "Mac menu must expose an app prerelease channel toggle"
grep -q 'Server prerelease' mac/Sources/PawTerm/MenuBarContent.swift || fail "Mac menu must expose a server prerelease channel toggle"
grep -q 'Mac App updates disabled for dev build' mac/Sources/PawTerm/MenuBarContent.swift || fail "Mac dev builds must explain that official updates are disabled"
