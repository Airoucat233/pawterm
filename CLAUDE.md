...
- `next`：集成和预发布分支。所有 feature 先合入 `next`，验证通过后发 prerelease。
- `main`：稳定发布分支。只从 `next` 合入确认过的版本，验证通过后发正式 release。
...
feature 验证通过后，提交并合入 `next`。不得从 feature 直接合入 `main`。

#### 2. next 分支：集成与 prerelease

`next` 是预发布分支。feature 合入后先跑完整验证：
...
`release:pre` 和 `release:server:pre` 只能在 `next` 执行。不要从 feature 分支发 prerelease。

#### 3. main 分支：稳定 release

确认 `next` 的 prerelease 可用后，才把 `next` 合入 `main`：
...
`release` 和 `release:server` 只能在 `main` 执行。`main` 只接受从 `next` 合并，不直接接 feature。
...
bash server/scripts/publish.sh --prerelease # next 分支：交互式 bump → push → npm publish prerelease dist-tag
```
